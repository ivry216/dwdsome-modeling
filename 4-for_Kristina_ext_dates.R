...
max_number_of_days <- difftime(max(initial_timestamps), min(initial_timestamps), units = "days")
# The prediction delta
prediction_delta_days <- 30
# Number of prediction days inside the observation interval
number_of_predictions <- floor(max_number_of_days / prediction_delta_days) + 1
# Timestep
timestep <- 6
# Timesteps in week
timesteps_in_delta_days <- floor(24 * prediction_delta_days / timestep) + 1
# First week timepoints
first_delta_times <- lapply(seq(timesteps_in_delta_days), function(x) {initial_timestamps[1] + (x - 1) * hours(timestep)})

# Timestamps by week
# Initialize
timestamps_by_shift <- list()
for (i_shift in seq(timesteps_in_delta_days)) {
  # Improve the index
  i_improved <- i_shift - 1
  # These timestamps
  these_timestamps <- list()
  # For all weeks
  for (j_week in seq(number_of_predictions)) {
    # Improve the index
    j_improved <- j_week - 1
    # Make timestamp
    these_timestamps[[j_week]] <- first_delta_times[[i_shift]] + j_improved * days(prediction_delta_days)
  }
...
if (TRUE) {
  for (i in seq(4)) {
    for (j in seq(generating_timestamps_num)) {
      print(paste("Line", i, ", sample:", j))
      write.xlsx(generated_dfs_lines[[i]][[j]], file = paste0(prediction_delta_days, "_tax_lvl_", taxonomy_level, "_line_", i, "_sample_", j, ".xlsx"))  
    }
  }
}